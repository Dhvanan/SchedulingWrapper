...
def subsetcore(hosts,req):
	...
	host_list ={}
	for host in hosts:
		host_list[host['host']] = host['free_vcpus']*cpu_allocation_ratio
	...
	for vm_type in req:
		hosts_dict = {}
		for request in req[vm_type]:
			core_req = config.flavors[request['flavor']][2]
			#core_req = flavors[request['flavor']][2]
			requests.append(core_req)
